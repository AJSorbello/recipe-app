...
def recipe_list(request):
    ...
    if request.method == "POST":
        ...
        if show_all:
            ...
            recipe_df_html = recipe_df.to_html(
                index=False, columns=["id", "name", "cooking_time", "difficulty"]
            )
        else:
            ...
            if qs.exists():
                ...
                print(
                    f"DataFrame: {recipe_df[['id', 'name', 'cooking_time', 'difficulty']]}"
                )
    ...
    if chart_type:
        if show_all:
            chart = get_chart(
                chart_type,
                all_recipe_df,
                labels=all_recipe_df["name"].tolist(),
                cooking_times=all_recipe_df["cooking_time"].tolist(),
            )
        else:
            if not recipe_df.empty:
                chart = get_chart(
                    chart_type,
                    recipe_df,
                    labels=recipe_df["name"].tolist(),
                    cooking_times=recipe_df["cooking_time"].tolist(),
                )
            else:
                chart = get_chart(
                    chart_type,
        ...
        print(
            f"Generated Chart: {chart[:100]}..."
        )  # Print only the first 100 characters of the chart
